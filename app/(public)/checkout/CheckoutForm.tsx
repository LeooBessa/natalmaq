...
export function CheckoutForm({ userId, cliente }: Props) {
  ...
  // Auto-calcular frete se endereço já estava preenchido
  useEffect(() => {
    if (cliente?.endereco?.cep && onlyDigits(cliente.endereco.cep).length === 8) {
      calcularFrete();
    }
          ...
          {cupom ? (
            <div className="flex items-center justify-between rounded border border-ok/40 bg-ok/10 px-3 py-2 text-sm">
              <div>
                ...
                {cupom.descricao && (
                  <span className="ml-2 text-xs text-white/60">{cupom.descricao}</span>
                )}
              </div>
            ...
            <div>
              <div className="mb-1 font-mono text-[10px] uppercase tracking-mono text-white/50">
                CUPOM DE DESCONTO
              </div>
              <div className="flex gap-2">
                <input
